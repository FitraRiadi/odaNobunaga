...
// NOTE: vendor asli import dari 'motion/react' — isinya identik dengan
// 'framer-motion' yang sudah terpasang, jadi diarahkan ke sini agar tidak
// ada dependency ganda.
...
export default function RubberSegment({
  ...
  // NOTE (adaptasi lokal): perubahan value dari luar (mis. scroll-spy)
  // dianimasikan rubber seperti tap — bukan jump instan — biar perpindahan
  // nav aktif kerasa smooth. onChange TIDAK dipanggil di sini (anti loop).
  useEffect(() => {
    if (!drag.current && committed.current !== index) {
      const from = committed.current;
      committed.current = index;
      if (reduce) jumpTo(index);
